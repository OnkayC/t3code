...
const INTERACTION_MODE_CHOICES: ReadonlyArray<{
  ...
  { mode: "plan-paused", label: "Plan paused" },
];

function planWorkflowLabel(workflow: ProviderPlanWorkflow): string {
  return workflow === "parallel" ? "Parallel" : "Iterative";
}
...
export function ThreadSettingsSheet(props: {
  ...
  readonly planWorkflow: ProviderPlanWorkflow | null;
  ...
  const displayedPlanWorkflow =
    props.planWorkflow ?? props.defaultPlanWorkflow ?? props.supportedPlanWorkflows[0];
      ...
      : submenu?.kind === "interaction"
        ? {
            ...
            rows: INTERACTION_MODE_CHOICES.filter((choice) =>
              props.supportedInteractionModes.includes(choice.mode),
            ).map((choice) => ({
              id: choice.mode,
              label: choice.label,
        ...
        : submenu?.kind === "workflow"
          ? {
              title: "Plan workflow",
              rows: props.supportedPlanWorkflows.map((workflow) => ({
                id: workflow,
                label: planWorkflowLabel(workflow),
                selected: workflow === displayedPlanWorkflow,
                onPress: () => {
                  void Haptics.selectionAsync();
      ...
      <View className="flex-1 justify-end">
        ...
        >
          ...
          <View style={{ paddingBottom: insets.bottom + 12 }}>
            ...
            />
            {props.supportedInteractionModes.length > 0 ? (
              <DisclosureRow
                label="Interaction"
                value={
            ...
            {props.interactionMode === "plan" && props.supportedPlanWorkflows.length > 0 ? (
              <DisclosureRow
                label="Plan workflow"
                value={displayedPlanWorkflow ? planWorkflowLabel(displayedPlanWorkflow) : undefined}
                onPress={() => setSubmenu({ kind: "workflow" })}
              />
            ) : null}
            <Pressable
              accessibilityRole="button"
